...
test.describe.serial(
  `OSD Wizard CCS AWS validation tests (${configName}) - OCP-54134,OCP-73204`,
  ...
  () => {
    ...
    test(`Cloud provider field validations`, async ({ createOSDWizardPage }) => {
      ...
      if (awsAccessKey) {
        ...
        await createOSDWizardPage.awsAccessKeyInput().clear();
      }
      await createOSDWizardPage.wizardNextButton().click();
      await createOSDWizardPage.isTextContainsInPage(
      ...
      if (awsAccountID && awsAccessKey && awsSecretKey) {
        ...
        await createOSDWizardPage.awsAccessKeyInput().clear();
        await createOSDWizardPage.awsAccessKeyInput().fill(awsAccessKey);
        await createOSDWizardPage.awsSecretKeyInput().clear();
        await createOSDWizardPage.awsSecretKeyInput().fill(awsSecretKey);
      }
    ...
    test(`Cluster details field validations`, async ({ page, createOSDWizardPage }) => {
      ...
      await createOSDWizardPage.useCustomKMSKeyRadio().check();
      await createOSDWizardPage.wizardNextButton().click();
      await createOSDWizardPage.isTextContainsInPage(
        ClustersValidation.ClusterSettings.Details.AWS.KeyARNs.EmptyValueError,
      );

      await createOSDWizardPage.keyArnInput().clear();
      await createOSDWizardPage
        .keyArnInput()
      ...
      await createOSDWizardPage.useDefaultKMSKeyRadio().check();
    ...
    test(`Cluster autoscaling validations`, async ({ createOSDWizardPage }) => {
      ...
      await createOSDWizardPage.clusterAutoscalingBalancingIgnoredLabelsInput().clear();
      await createOSDWizardPage.clusterAutoscalingBalancingIgnoredLabelsInput().fill('test,test,');
      await createOSDWizardPage.clusterAutoscalingBalancingIgnoredLabelsInput().blur();
      await createOSDWizardPage.isTextContainsInPage(
        ClustersValidation.ClusterSettings.Machinepool.Common.ClusterAutoscaling
      ...
      await createOSDWizardPage
        ...
        .press('Control+a');
      await createOSDWizardPage.clusterAutoscalingScaleDownUtilizationThresholdInput().fill('1.5');
      ...
      await createOSDWizardPage
        .clusterAutoscalingScaleDownUtilizationThresholdInput()
        .press('Control+a');
      await createOSDWizardPage.clusterAutoscalingScaleDownUtilizationThresholdInput().fill('0.5');
      await createOSDWizardPage.clusterAutoscalingScaleDownUtilizationThresholdInput().blur();
      await createOSDWizardPage.isTextContainsInPage(
        ClustersValidation.ClusterSettings.Machinepool.Common.ClusterAutoscaling.NegativeValueError,
        false,
      ...
      await createOSDWizardPage.clusterAutoscalingScaleDownDelayAfterAddInput().press('Control+a');
      ...
      await createOSDWizardPage
        ...
        .press('Control+a');
      await createOSDWizardPage.clusterAutoscalingScaleDownDelayAfterDeleteInput().fill('10milli');
