...
const testData = require('../../fixtures/osd-gcp/osd-ccs-gcp-ystream-channel.spec.json');
...
const configName = `${clusterProperties.CloudProvider}-${clusterProperties.SubscriptionType}-${clusterProperties.InfrastructureType}-${clusterProperties.AuthenticationType}`;
...
test.describe.serial(
  ...
  { tag: ['@advanced', '@day1', '@ystream', '@osd', '@gcp'] },
  () => {
    ...
    test('Cloud provider', async ({ createOSDWizardPage }) => {
      test.skip(
        !QE_GCP_WIF_CONFIG,
        'Set QE_GCP_WIF_CONFIG in playwright.env.json (GCP WIF config).',
      );

      await createOSDWizardPage.completeGcpCcsWifCloudProviderStep(
