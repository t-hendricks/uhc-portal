...
class CreateOSDCluster extends Page {
  async isCreateOSDPage() {
    ...
    return (
      URL === 'https://qa.foo.redhat.com:1337/openshift/create/osd' ||
  ...
  async isCreateOSDTrialPage() {
    const URL = await browser.getUrl();
    return (
      URL === 'https://qa.foo.redhat.com:1337/openshift/create/osdtrial?trial=osd' ||
  ...
  get awsProvider() {
    return $('div[data-testid="aws-provider-card"]');
  }

  get CCSRadiobutton() {
    return $('input#byoc-true');
  }
  ...
  get awsAccessKeyInput() {
    return $('input#access_key_id');
  }

  get awsSecretAccessKeyInput() {
    return $('input#secret_access_key');
  }

  get TrialSelected() {
    return $("input:checked[name='billing_model'][value='standard_trial']");
  }

  get fakeClusterBanner() {
    return $("//div[contains(text(), 'On submit, a fake OSD cluster will be created')]");
  }
  ...
  get clusterNameInputError() {
    return $('input#name ~ div.pf-m-error');
  }
  ...
  get publicClusterRadiobutton() {
    return $('input#cluster_privacy-external');
  }

  get privateClusterRadiobutton() {
    return $('input#cluster_privacy-internal');
  }

  get installIntoVPCCheckbox() {
    return $('input#install_to_vpc');
  }

  get usePrivateLinkCheckbox() {
    return $('input#use_privatelink');
  }
}
