...
That will start the the Webpack development server. See below for instructions regarding the "chromed environment" in order to interact with the user interface.
...
Then in another terminal, run `yarn startmock` instead of `yarn start`, and you'll get a working UI showing mock data running with the chromed environment.
...
Make sure to start the backend proxy server before starting the insights proxy.
....
$ ./backend
....
