...
The backend proxy doesn't yet work correctly with sso.redhat.com, so for now
to obtain the access token go to the
https://cloud.stage.openshift.com/openshift/token.html[old token page] and copy the
_offline token_.
