...
class CostSummary extends Component {
  ...
  formatPercentage = (value = 0) => value.toLocaleString('en', {
    style: 'percent',
  ...
  getClusters = () => {
    ...
    return report && report.data.map(data => data.clusters.map(cluster => cluster.values.map((value) => {
      ...
      const val = `${this.formatCurrency(cost, units)} (${this.formatPercentage(percentage)})`;
