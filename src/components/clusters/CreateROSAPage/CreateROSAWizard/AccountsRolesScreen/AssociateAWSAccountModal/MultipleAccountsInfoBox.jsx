...
const MultipleAccountsInfoBox = ({ setIsAlertShown, ocmRole, userRole }) => {
  const associateMultRoles =
    'You can associate multiple AWS accounts to create ROSA clusters on any of the associated AWS accounts from your organization.';
  let specifyProfile = '';
  if (ocmRole || userRole) {
    specifyProfile = `When creating the ${
      ocmRole ? 'OCM' : 'user'
    } role, you may specify an AWS account profile.`;
  }
  return (
    <Alert
      variant="info"
