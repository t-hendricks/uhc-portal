...
describe('<InsightsAdvisor />', () => {
  const props = {
    entries: [['1', 0], ['2', 1], ['3', 0], ['4', 2]],
    issueCount: 3,
    externalId: 'foo-bar',
  };
  const widgetWrapper = shallow(<InsightsAdvisor insightsData={fixtures.insightsData} externalId="foo-bar" />);
  ...
  /** TODO: Uncomment when OCP Advisor is in non-beta
    ...
    it('should render title links to OCP Advisor', () => {
      ...
      links.forEach((link, index) => {
        expect(link.props().href).toBe(`http://localhost/openshift/insights/advisor/clusters/foo-bar?total_risk=${index + 1}`);
      });
    });
