...
const getClusterIdentityProviders = (clusterID) => (dispatch) =>
  dispatch({
    ...
    payload: clusterService.getIdentityProviders(clusterID).then((response) => {
      response.data.items = get(response.data, 'items', []);
      return response;
    }),
  });

const createClusterIdentityProvider = (clusterID, params) => (dispatch) =>
  dispatch({
...
const editClusterIdentityProvider = (clusterID, params) => (dispatch) =>
  dispatch({
