...
const getOCMRoles = subID => dispatch => dispatch({
  type: OCMRolesConstants.GET_OCM_ROLES,
  payload: accountsService.getSubscriptionRoleBindings(subID),
});

const grantOCMRole = (subID, username, roleID) => dispatch => dispatch({
...
const editOCMRole = () => dispatch => dispatch({
  type: OCMRolesConstants.EDIT_OCM_ROLE,
});

const deleteOCMRole = (subID, roleBindingID) => dispatch => dispatch({
