...
test('filterPersistentStorageValuesByQuota works without currentValue and with positive quota', () => {
  ...
  const persistentStorageValues = { values: [{ unit: 'B', value: 100 * 2 ** 30 }, { unit: 'B', value: 600 * 2 ** 30 }, { unit: 'B', value: 1100 * 2 ** 30 }] };
  expect(filterPersistentStorageValuesByQuota(null, persistentStorageValues, remainingQuota)).toEqual({ values: [{ unit: 'B', value: 100 * 2 ** 30 }, { unit: 'B', value: 600 * 2 ** 30 }] });
});
...
test('filterPersistentStorageValuesByQuota works with currentValue and with zero remaining quota', () => {
  ...
  expect(filterPersistentStorageValuesByQuota(currentValue, persistentStorageValues, remainingQuota)).toEqual({ values: [{ unit: 'B', value: 100 * 2 ** 30 }, { unit: 'B', value: 600 * 2 ** 30 }] });
});
