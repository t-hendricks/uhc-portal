...
import { useCreateEditHtpasswdUser } from './useCreateEditHtpasswdUser';
...
describe('useCreateHtpasswdUser', () => {
  ...
  describe('Add new user', () => {
    it('sends correct information when making API call', async () => {
      ...
      const { result } = renderHook(() => useCreateEditHtpasswdUser(clusterId, idpId));
    ...
    it('makes regionalized call when region is passed', async () => {
      mockAddUser.mockResolvedValue('success');
      const { result } = renderHook(() => useCreateEditHtpasswdUser(clusterId, idpId, region));
