...
const getIdSlices = (itemId: string, filterText: string): IdSlice[] => {
  ...
  lcs.forEach(({ beg, end }) => {
    idSlices.push({ text: itemId.slice(pos, beg), isBold: false });
    idSlices.push({ text: itemId.slice(beg, end + 1), isBold: true });
    pos = end + 1;
  });
  if (pos < itemId.length) {
    idSlices.push({ text: itemId.slice(pos), isBold: false });
  }
  return idSlices;
};
